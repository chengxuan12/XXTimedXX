...
$.fn.datepicker = function(setter) {
    $(this).each(function() {
        ...
        var setting = $.extend({
            ...
            onSelect: function() {
                if($("#tmpDate")) {
                    $("#tmpDate").val(this.inputField.value);
                    $("#tmpDate").click();
                }
                this.hide();
            },
            onChange:function(){
            	if(setter!=undefined && setter.change!=undefined){
            	   setter.change();
            	}
            }
        },
        setter);
        /*if (setter && setter.dateFormat) {
            setting.dateFormat = setter.dateFormat;
        }*/
        if (setter && setter.seconds) {
            setting.dateFormat = "%Y-%m-%d %H:%M:%S";
            setting.showTime = (setting.showTime == undefined ? 24: 12);

        }
        Calendar.setup(setting);
    });
    if(isIE6){   
    	$('<iframe id="iframe_for_datepicker" style="position:absolute;z-index:9999;display:none;"> </iframe>').appendTo('body')
    	setInterval(function(){
    		var table=$('table.DynarchCalendar-topCont:visible');
    		if(table.length>=1)
    		{
    			$('#iframe_for_datepicker').css({
    						'width'  : table.width()+'px',
    						'height' : table.height()+'px',
    						'left'   : table.get(0).style.left,
    						'top'    : table.get(0).style.top,
    						'display': ''
    			});
    		}
    		else{
    			$('#iframe_for_datepicker').css({'display':'none'});
    		}
    	},1);
    }
